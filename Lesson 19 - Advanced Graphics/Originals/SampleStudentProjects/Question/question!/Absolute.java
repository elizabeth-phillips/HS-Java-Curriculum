...
public class Absolute extends Applet implements Runnable
{
  ...
  int		smax, bmax;
  int		scur, renew, rcnt=0, sstretch, txtalign=100;
  long		score;
  double bcur;
  // Sounds
  SoundClip	blast, crash, kill;
  SoundClip music;
  ...
  {
    return("Question! - by Josh Whitehead, Philip Mena, and Alex Ostermann");
  }
  ...
  public void init()
  {
    ...
    // Audio
      blast = new SoundClip("blast.au");
      crash = new SoundClip("collisn.au");
      kill = new SoundClip("mdestr.au");

    blast.play();  blast.stop();
    crash.play();  crash.stop();
    kill.play();   kill.stop();
    music = new SoundClip("music.au");
...
// This creates the starfield in the background
  ...
  public void ShowScore()
  {
    ...
    goff.setFont(smallfont);
    s="laser: "+bcur+"/"+bmax;
    goff.fillRect(txtalign, my-10, (int)(bcur*sstretch), 10);
